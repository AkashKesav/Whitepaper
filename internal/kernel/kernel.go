...
type Kernel struct {
	...
	// Hot Path (instant retrieval)
	hotCache *memory.HotCache

	// Cold Path (batch summarization)
...
func (k *Kernel) Start() error {
	...
	// Initialize Hot Path (embedding service + hot cache)
	embedService := embedding.New(k.config.AIServicesURL, k.logger)
	k.hotCache = memory.NewHotCache(embedService, k.logger)
	k.logger.Info("Hot cache initialized for instant retrieval")

	// Initialize Cold Path (message batcher for summarization)
	k.batcher = memory.NewBatcher(k.config.AIServicesURL, k.logger, func(userID, summary string, messages []memory.Message) error {
		k.logger.Info("Cold path summary generated",
			zap.String("user_id", userID),
			zap.Int("messages", len(messages)),
			zap.Int("summary_len", len(summary)))
		// The summary will be used by reflection engine to create insights
		// For now, just log it - full integration would pass to graph
		return nil
	})
	k.batcher.Start()
	k.logger.Info("Cold path batcher started")
	...
	k.policyEngine = policy.NewEngine()
	k.logger.Info("Policy Engine initialized")
...
func (k *Kernel) runIngestionLoop() {
	...
	sub, err := k.jetStream.Subscribe("transcripts.*", func(msg *nats.Msg) {
		...
		if err := k.ingestionPipeline.Process(k.ctx, msg.Data); err != nil {
			...
			// Feed to Cold Path batcher for summarization
			if k.batcher != nil {
				var event graph.TranscriptEvent
				if json.Unmarshal(msg.Data, &event) == nil {
					k.batcher.Add(event.UserID, event.UserQuery, event.AIResponse)
				}
			}
...
// HotCache returns the hot cache for direct access (used by agents)
func (k *Kernel) HotCache() *memory.HotCache {
	return k.hotCache
}

// GraphClient returns the graph client for direct access (used by traversal operations)
func (k *Kernel) GraphClient() *graph.Client {
	return k.graphClient
}

// StoreInHotCache stores a message in the hot cache for instant retrieval
