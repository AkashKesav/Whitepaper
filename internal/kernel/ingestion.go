...
func PublishTranscript(js nats.JetStreamContext, event *graph.TranscriptEvent) error {
	...
	// Sanitize userID for NATS subject (. is delimiter, * and > are wildcards)
	sanitizedUserID := strings.ReplaceAll(event.UserID, ".", "_")
	sanitizedUserID = strings.ReplaceAll(sanitizedUserID, "@", "_")
	sanitizedUserID = strings.ReplaceAll(sanitizedUserID, "*", "_")
	sanitizedUserID = strings.ReplaceAll(sanitizedUserID, ">", "_")
	subject := fmt.Sprintf("transcripts.%s", sanitizedUserID)
